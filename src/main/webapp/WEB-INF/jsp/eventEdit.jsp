...
<c:choose>
	<c:when test="${ event.stateId == 1 }"><c:set var="available"    value="${false}"/></c:when>
			...
			<c:forEach items="${ editEventForm.getUserFriends() }" var="friend" varStatus="i">                    
                    ...
                    <c:choose>
                    	<c:when test="${ isOrganizer }">
                    		<td><form:checkbox path="userFriends[${i.index}].invited"  class="invite" /><span style="margin-left:20px">${friend.getEmail()}</span> </td>
                    	...
                    	<c:when test="${ friend.getInviter() eq organizerEmail }">
                    		<td><c:choose>
                    				<c:when test="${ friend.invited }">
                    					<form:hidden path="userFriends[${i.index}].invited" value="true"/>
                    			...
                    			<form:hidden  path="userFriends[${i.index}].allowed"  /><span style="margin-left:20px">${friend.getEmail()}</span> 
                    		</td>
                    	</c:when>
                    	<c:when test="${ friendUserEmail eq organizerEmail }">
                    		<td><form:hidden path="userFriends[${i.index}].invited"  value="true"  />
                    			<i style="color: gold; margin-right:20px" class="glyphicon glyphicon-star"></i>
                    			<form:hidden  path="userFriends[${i.index}].allowed"  value="true" />${organizerEmail}
                    		</td>
                    	</c:when>
                    	<c:otherwise>
                    		<td><form:checkbox path="userFriends[${i.index}].invited" class="invite" />
                    			<form:hidden  path="userFriends[${i.index}].allowed" />
                    			<span style="margin-left:20px">${friend.getEmail()}</span>
                    		</td>
                    	</c:otherwise>
                    </c:choose>
					<td><form:hidden path="userFriends[${i.index}].email" value="${ friend.getEmail() }" /> 
					<c:choose >
